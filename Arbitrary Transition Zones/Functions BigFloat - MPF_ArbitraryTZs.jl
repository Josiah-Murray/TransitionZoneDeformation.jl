...
Many of the functions here are redundant and don't need BigFloat versions.
To make the implementation quicker, however, I did it anyway.
...
function AddConsistentTZBF(location, previousTZ::TransitionZoneBF, k_right, C_right)
  return TransitionZoneBF(location, previousTZ.k_right, k_right, previousTZ.C_right, C_right)
end
...
function CalcDynamicDeformationBF(xVals,tVals, parameters, inversionMethod)

  println(inversionMethod)
  deformations = invertLaplaceBF(xVals, tVals, LaplaceSpaceFunctionBF, parameters, inversionMethod)
...
function invertLaplaceBF(xVals,tVals,LaplaceSpaceFunction, parameters, inversionMethod)
...
function LaplaceSpaceFunctionBF(x,s, parameters)
  ...
  rVals = RValuesBF(s, EI, m, C, k)
  r1, r2, r3, r4 = rVals


  #||--Find b values--||#
  #Solve for the undetermined coefficients by solving a linear system
  b1,b2,b3,b4 = CoefficientSolverBF(x, s, parameters)

  #||--Construct Laplace-space function--||#

  ŷ = b1*exp(r1*x) + b2*exp(r2*x) + b3*exp(r3*x) + b4*exp(r4*x) + (P/abs(v))*(exp(-s*x/v)  /  (  ( (EI*s^4)/v^4 ) +m*s^2 + C*s + k )  )*HeavisideBF((x-xp)/v)
...
function CoefficientSolverBF(x, s, parameters)
  ...
  LHSMatrix = [ [0 1 0 0 ; 0 0 1 0 ]  zeros(Complex{BigFloat},2, numRows - 4) ]



  #RHS Vector for system.
  RHS = zeros(Complex{BigFloat},numRows)
  ...
  for i in eachindex(xtz_list)
    ...
    if(!pointForceAdded)
      ...
      if(xp<tz.location)
        ...
        LHSMatrix = [LHSMatrix;     zeros(Complex{BigFloat}, 4, 4*(segment-1))      continuitySubMatrix     zeros(Complex{BigFloat}, 4, numRows-4*(segment-1)-8)  ]
    ...
    lr1, lr2, lr3, lr4 = RValuesBF(s,EI,m, tz.C_left, tz.k_left)
    rr1, rr2, rr3, rr4 = RValuesBF(s,EI,m, tz.C_right, tz.k_right)
    ...
    #RHS Vector
    for i = 1:4
      RHS[2 + (segment-1)*4 + i] = (-s/v)^(i-1)*( P*exp(-s*tz.location/v)/abs(v)  )*(  1/( ( (EI*s^4)/v^4  ) +m*s^2 + tz.C_left*s + tz.k_left   ) - 1/( ( (EI*s^4)/v^4  ) +m*s^2 + tz.C_right*s + tz.k_right   )   )*HeavisideBF((tz.location-xp)/v)
    end
  ...
  LHSMatrix = [LHSMatrix; zeros(Complex{BigFloat},2, numRows - 4) [ 1 0 0 0 ; 0 0 0 1] ]
...
function SteadyStateTravellingSolutionBF(xVals, tVals, parameters, ks, C)
  EI, m, xp, xtz_list, P, v = parameters
...
function CriticalVelocityBF(EI, m , k)
  v_cr = (4*k*EI/(m^2))^(1/4)
  return v_cr
end
