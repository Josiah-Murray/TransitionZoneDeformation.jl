...
function TestGWRImplementation(xVals,tVals,LaplaceSpaceFunction, parameters)
  ...
  for xi in eachindex(xVals)
    ...
    for ti in eachindex(tVals)
      ...
      deformations[ti,xi] = my_gwr(s -> real(LaplaceSpaceFunction(x,s,parameters)), t, 40)
...
inversionMethod = (xVals, tVals,LaplaceSpaceFunction, parameters) -> TestGWRImplementation(xVals, tVals,LaplaceSpaceFunction, parameters)
deformations_gwr = @time CalcDynamicDeformation(xVals_fast, tVals10_fast, parameters, inversionMethod)
Graph10Times(deformations_gwr,xVals_fast,tVals10_fast,parameters,SteadyStateDeformations, false, colours, "", graphName)
